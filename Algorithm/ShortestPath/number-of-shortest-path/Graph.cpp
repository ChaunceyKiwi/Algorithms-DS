...
*/
void Graph::calculate_shortest_path_from_to(int index, int target) {
  // Initialization
  set<int> S;
  S.insert(index);
  int count = 0;
        ...
        for (pair<int, float> cost : this->adjacencyLists[index1]) {
          if (S.count(cost.first) == 0) {
            if (distance[index1] != INT_MAX) {
              if (distance[index1] + cost.second == minimumDis.second && cost.first == target) {
                count++;
              }
              if (distance[index1] + cost.second < minimumDis.second) {
                if (cost.first == target) {
                  count = 1;
                }
  ...
  // Results
  cout << "There are " << count <<" shortest paths from node" << index << " to node" << target << "."<< endl;
}
